...
export const saveTourDaily = async (
  ...
  productTours.forEach((it, idx) => {
    ...
    if (idx > 0) {
      const curCity = products[idx].nameAreas[0].pOIDistrictName;
      const preCity = products[idx - 1].nameAreas[0].pOIDistrictName;
      if (curCity !== preCity) {
        // 计算产品之间的流转行程
    ...
    const curTours = it.tourDaily.tourInfo.tourDailyDescriptions;
    // 去掉中间段行程中的返程字样
    curTours[curTours.length-1].dailyDescription = curTours[curTours.length-1].dailyDescription.replace(/(-)?返程/, "");
    // 组合中产品最后一日的酒店默认和前一晚一样
    if (curTours.length >= 2) {
      const curDay = curTours[curTours.length - 1];
      // 当前日没有酒店
      if (
        !curDay.tourDailyInfos.find(
          (it) => it.activeType.name === "酒店" && !!product?.sameHotel,
        )
      ) {
        const hotel = curTours[curTours.length - 2].tourDailyInfos.find(
          (it) => it.activeType.name === "酒店",
        );
        curDay.tourDailyInfos.push(hotel);
      }
    }
