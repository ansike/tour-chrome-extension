...
import {
  getProductImageText,
  saveProductBaseInfo,
} from "../../scripts/savedescriptioninfo";

export const saveProductRichtext = async (
  productId: string | number,
  products: any[],
) => {
  const productsRichtext = await Promise.all(
    products.map((pro) => getProductImageText(pro.baseInfo.productId)),
  );
  const curProductImageText = await getProductImageText(productId);
  const existsImgs = curProductImageText.productImages.map((it) => it.imageId);
  const cover = productsRichtext[0].productImages.find(
    (it) => it.accompanyTourInfo.imageType === "封面",
  );
  const map = {};
  // 子产品的图片放到新的产品上
  const productImages = productsRichtext
    .map((pro) =>
      pro.productImages
        .map((img) => {
          return {
        ...
        .filter((it) => {
          if (existsImgs.includes(it.imageId)) {
            return false;
          }
          if (map[it.imageId]) {
            return false;
