...
export const saveProductResource = async (
  ...
  const productsSegment = await Promise.all(
    productObjs.map((pro) => getSegments(pro.baseInfo.productId)),
  );
  ...
  for (let i = 0; i < productsSegment.length; i++) {
    ...
    if (product?.sameHotel) {
      ...
      const idx = seg.productSegments.segments.findLastIndex((s) => s.hotel);
      const s = seg.productSegments.segments[idx];
      s.segmentBase.maxStayNights += 1;
      s.segmentBase.minStayNights += 1;
      s.segmentBase.stayNights += 1;
    } else {
      // 如果不是同一个酒店，当前行程段的最后一晚不变，增加一晚行程段酒店
      // 上一段行程的最后一个segment
      // const prevSeg =
      //   productsSegment[i - 1].productSegments.segments[
      //     productsSegment[i - 1].productSegments.segments.length - 1
...
function getRandomDate() {
  ...
  while (randomDates.length < 3) {
    const randomTimestamp =
      Math.random() * (futureDate.getTime() - today.getTime()) +
      today.getTime();
