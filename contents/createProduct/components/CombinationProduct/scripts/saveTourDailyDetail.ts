import { getProductTourInfoList, getTourDaily } from "../../scripts/getProductBaseInfo"
                    ...
                    "ranges": [
                        {
                            "rangeKey": "valueRange",
                            "name": "地点标签范围",
                            "value": "2006,25831,65,24,11,703,25810,7,8,16,19,142,4841,20521,20549,20565,20572,28,32,140,602,20531,45,21,20,137,20571,20566,63,10,11,4836,17,703,147,148,755,37,25710,20631",
                            "enable": true
                        }
                    ],
                    "require": false
...
export const saveTourDaily = async (productId: string | number, products: any[]) => {
    ...
    productTours.forEach((it, idx) => {
        if (idx > 0) {
            ...
            const tourDailyHotels = productTours[idx].tourDaily.tourInfo.tourDailyDescriptions[0].tourDailyInfos.find(it => it.activeType.name === '酒店').tourDailyHotels
            const transTour = calcTransDay(from, to, tourDailyHotels)
            middleTourDailyDescriptions.push(transTour)
        }
        middleTourDailyDescriptions.push(...it.tourDaily.tourInfo.tourDailyDescriptions)
    })
...
export const saveTourDailyDetail = async (tourInfo) => {
    const res = await fetch("https://online.ctrip.com/restapi/soa2/20049/saveTourDailyDetail.json", {
        "headers": {
            "accept": "*/*",
            "accept-language": "zh-CN,zh;q=0.9",
...
export const calculateTourInfoScore = async (tourInfo) => {
    const res = await fetch("https://online.ctrip.com/restapi/soa2/20049/calculateTourInfoScore", {
        "headers": {
            "accept": "*/*",
            "accept-language": "zh-CN,zh;q=0.9",
        ...
        "referrerPolicy": "no-referrer-when-downgrade",
        "body": JSON.stringify({
            businessData: "{}", contentType: "json", requestHeader: { locale: "zh-CN" },
            tourInfo
        }),
        "method": "POST",
        "mode": "cors",
