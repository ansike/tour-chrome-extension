...
export const saveSubProductResource = async (productId: string, sub: any) => {
  ...
  let destinationCity =
    draftProductSegments.segments[0].segmentBase.departureCity;
  ...
  if (draftProductSegments.segments[0].segmentBase.departureCity.cityId !== 0) {
    ...
    destinationCity =
      draftProductSegments.segments[0].segmentBase.destinationCity;
  }

  if (
    draftProductSegments.segments[draftProductSegments.segments.length - 1]
      .segmentBase.destinationCity.cityId !== 0
  ) {
    // 在最后增加子产品一条行程
    await saveSegment({
      segmentBase: {
        ...
        departureCity:
          draftProductSegments.segments[
            draftProductSegments.segments.length - 1
          ].segmentBase.destinationCity,
  ...
  // 拥有机场的城市
  const cities = getTransitionCitiesByType(
    multiDepartureCities,
    transitionType,
  );
  // 不包含当前的城市
  let filteredCities = cities.filter(
    (item) => item.cityId !== destinationCity.cityId,
  );
  console.log("filteredCities", filteredCities);
  // 保存当前城市
  await saveSegmentCommonData(productId, filteredCities);

  async function setMultipleDepartureCities(productId: string) {
    // 重新随机日期、提交分段
    ...
    const checkSegmentResultCities = async (productId: string) => {
      ...
      while (isChecking) {
        ...
        if (res.result === "F") {
          isChecking = false;
          const errorCityIds = res.checkSegmentResultCities.map(
            (it) => it.city.cityId,
          );
          const newFilteredCities = filteredCities.filter(
            (item) => !errorCityIds.includes(item.cityId),
          );
          await sleep(1000);
          console.log(productId, { filteredCities: newFilteredCities });
          isChecking = false;
          // 检索到的城市无资源考虑重新设置出发城市
          if (!filteredCities.length) {
            return await setMultipleDepartureCities(productId);
          } else {
            await saveSegmentCommonData(productId, newFilteredCities);
            await publishProductModules(productId);
          }
  ...
  await setMultipleDepartureCities(productId);
...
// 获取前一个产品的segments
export const saveSegmentCommonData = async (
  productId: string,
  departureCities,
) => {
  const body = {
  ...
  try {
    const res = await fetch(
      "https://online.ctrip.com/restapi/soa2/15638/SaveSegmentCommonData?_fxpcqlniredt=09031147210876379845&_fxpcqlniredt=09031147210876379845",
      {
        headers: {
          accept: "*/*",
          "accept-language": "en",
...
export const getMultiDepartureCities = async () => {
  const res = await fetch(
    "https://online.ctrip.com/restapi/soa2/15638/getMultiDepartureCities.json?_fxpcqlniredt=09031147210876379845&_fxpcqlniredt=09031147210876379845",
    {
      headers: {
        accept: "*/*",
        "accept-language": "en",
...
export const getSubmitSegmentsResult = async (productId: string) => {
  const res = await fetch(
    "https://online.ctrip.com/restapi/soa2/15638/getSubmitSegmentsResult?_fxpcqlniredt=09031147210876379845&_fxpcqlniredt=09031147210876379845",
    {
      headers: {
        accept: "*/*",
        "accept-language": "en",
      ...
      referrer:
        "https://vbooking.ctrip.com/product/input/newResourceRule?productid=49492798&from=vbk",
      referrerPolicy: "no-referrer-when-downgrade",
...
export const publishProductModules = async (productId: string) => {
  const res = await fetch(
    "https://online.ctrip.com/restapi/soa2/15638/publishProductModules?_fxpcqlniredt=09031023410212933560&_fxpcqlniredt=09031023410212933560",
    {
      headers: {
        accept: "*/*",
        "accept-language": "zh-CN,zh;q=0.9",
      ...
      referrerPolicy: "no-referrer-when-downgrade",
      body: `{\"contentType\":\"json\",\"head\":{\"cid\":\"09031023410212933560\",\"ctok\":\"\",\"cver\":\"1.0\",\"lang\":\"01\",\"sid\":\"8888\",\"syscode\":\"09\",\"auth\":\"\",\"extension\":[]},\"productId\":${productId},\"module\":\"segment\",\"modifyUser\":\"vbk_1533099\"}`,
      method: "POST",
      mode: "cors",
      credentials: "include",
    },
